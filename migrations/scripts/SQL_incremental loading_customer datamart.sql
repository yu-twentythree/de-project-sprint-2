...
dwh_delta_insert_result AS ( -- делаем расчёт витрины по новым данным. Этой информации по заказчикам в рамках расчётного периода раньше не было, это новые данные. Их можно просто вставить (insert) в витрину без обновления
    SELECT  
            ...
            T4.customer_count_order AS customer_count_order,
            T4.customer_avg_price_order AS customer_avg_price_order,
            T4.product_type AS customer_top_product_category,
            T4.craftsman_id AS customer_top_craftsman_id,
            T4.customer_median_time_order_completed AS customer_median_time_order_completed,
            T4.customer_count_order_created AS customer_count_order_created,
            T4.customer_count_order_in_progress AS customer_count_order_in_progress,
            T4.customer_count_order_delivery AS customer_count_order_delivery,
            T4.customer_count_order_done AS customer_count_order_done,
            T4.customer_count_order_not_done AS customer_count_order_not_done,
                        ...
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у заказчика. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                                ...
                                COUNT(order_id) AS customer_count_order,
                                AVG(T1.product_price) AS customer_avg_price_order,                              
                                PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY diff_order_date) AS customer_median_time_order_completed,
                                SUM(CASE WHEN T1.order_status = 'created' THEN 1 ELSE 0 END) AS customer_count_order_created,
                                SUM(CASE WHEN T1.order_status = 'in progress' THEN 1 ELSE 0 END) AS customer_count_order_in_progress, 
                                SUM(CASE WHEN T1.order_status = 'delivery' THEN 1 ELSE 0 END) AS customer_count_order_delivery, 
                                SUM(CASE WHEN T1.order_status = 'done' THEN 1 ELSE 0 END) AS customer_count_order_done, 
                                SUM(CASE WHEN T1.order_status != 'done' THEN 1 ELSE 0 END) AS customer_count_order_not_done,
                                ...
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самого популярного мастера ручной работы. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
...
insert_delta AS ( -- выполняем insert новых расчитанных данных для витрины 
    INSERT INTO dwh.customer_report_datamart (
        ...
        customer_money, 
        platform_money, 
        customer_count_order, 
        customer_avg_price_order, 
        customer_median_time_order_completed,
        customer_top_product_category,
        customer_top_craftsman_id,
        customer_count_order_created, 
        customer_count_order_in_progress, 
        customer_count_order_delivery, 
        customer_count_order_done, 
        customer_count_order_not_done, 
        report_period
    ) SELECT 
            customer_id,
            customer_name,
...
update_delta AS ( -- выполняем обновление показателей в отчёте по уже существующим заказчикам
    UPDATE dwh.customer_report_datamart SET
        ...
        customer_count_order = updates.customer_count_order, 
        customer_avg_price_order = updates.customer_avg_price_order, 
        customer_median_time_order_completed = updates.customer_median_time_order_completed, 
        customer_top_product_category = updates.customer_top_product_category,
        customer_top_craftsman_id = updates.customer_top_craftsman_id,
        customer_count_order_created = updates.customer_count_order_created, 
        customer_count_order_in_progress = updates.customer_count_order_in_progress, 
        customer_count_order_delivery = updates.customer_count_order_delivery, 
        customer_count_order_done = updates.customer_count_order_done,
        customer_count_order_not_done = updates.customer_count_order_not_done, 
